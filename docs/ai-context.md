...
- ffmpeg macOS : `brew install ffmpeg` en CI produit un binaire lié dynamiquement aux `.dylib` du Cellar Homebrew (chemins absolus versionnés, ex. `/opt/homebrew/Cellar/ffmpeg/8.1.2/lib/libavdevice.62.dylib`). Copier ce binaire tel quel casse `dyld` sur toute machine où Homebrew ffmpeg est absent ou dans une version différente — donc chez la quasi-totalité des utilisateurs (vécu le 2026-08-31, erreur `Library not loaded`). Le CI passe le binaire à `dylibbundler` (`-od -b -x ffmpeg-<target> -d binaries/ffmpeg-libs -p @executable_path/../Resources/ffmpeg-libs/`) pour copier ses dépendances et réécrire ses load commands. `src-tauri/tauri.macos.conf.json` (auto-mergé par Tauri sur build macOS, cf. doc Tauri "Platform-specific Configuration") bundle ensuite `binaries/ffmpeg-libs/` dans `Contents/Resources/ffmpeg-libs/`. L'entitlement `disable-library-validation` (déjà appliqué au sidecar ffmpeg) couvre le chargement de ces dylibs re-signées ad-hoc par `install_name_tool`.
- ffmpeg Linux : même défaut que macOS (`cp $(which ffmpeg)` depuis `apt`, lié dynamiquement aux `.so` de la distro CI) — remplacé par le même mécanisme que Windows : téléchargement de `ffmpeg-master-latest-linux64-gpl.tar.xz` (build statique BtbN, tag `latest`, vérifié contre `checksums.sha256`). Confirmé par `objdump -p` : ce build ne dépend que de libs glibc baseline (`libc`, `libm`, `libpthread`, `libdl`, `librt`, `libgcc_s`, `ld-linux`) toujours présentes sur toute distro glibc — aucun bundling nécessaire contrairement à macOS. Le paquet apt `ffmpeg` a été retiré des dépendances système Linux du CI (ne servait qu'à sourcer ce binaire).
